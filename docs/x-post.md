...
I built Open Feed Recsys Lab: a tiny Codex/ClawHub skill that turns `xai-org/x-algorithm` into a commit-pinned local report.

It checks source, Git LFS/Phoenix artifacts, README/config mismatches, and generates a shareable architecture map.
...
5/ Use it with Codex, Claude Code, or ClawHub when you want a concrete local artifact before writing takes about the open algorithm.
...
If you starred `xai-org/x-algorithm` but have not run or mapped it yet, this gives you a clean first report in one command.
